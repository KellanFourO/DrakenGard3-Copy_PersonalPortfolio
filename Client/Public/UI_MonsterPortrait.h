...
class CUI_MonsterPortrait final : public CMyUI
{
public:
public:
...
private:
	CUI_MonsterPortrait(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LEVEL eCurrentLevel);
	CUI_MonsterPortrait(const CUI_MonsterPortrait& rhs);
	virtual ~CUI_MonsterPortrait() = default;
...
private:
	MONSTER_PORTRAIT				m_tInfo = {};
	_float					m_fCurrentHp;
	_float					m_fPrevHp;
	/*_float					m_fHPBarTimeDuration = 100.f;*/

	const _float			m_fDefaultScale = 0.4f;
...
public:
	static CUI_MonsterPortrait*	Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LEVEL eCurrentLevel); //! 원형객체 생성
