#include "stdafx.h"
#include "..\Public\Effect_Dust.h"

#include "GameInstance.h"


CEffect_Dust::CEffect_Dust(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CAlphaObject(pDevice, pContext)
{

}

CEffect_Dust::CEffect_Dust(const CEffect_Dust & rhs)
	: CAlphaObject(rhs)
{
}

HRESULT CEffect_Dust::Initialize_Prototype()
{
	/* 원형객체의 초기화과정을 수행한다. */
...
HRESULT CEffect_Dust::Initialize(void* pArg)
{	
	...
	m_tEffectDesc.vCreatePos.y += 1.5f;
...
void CEffect_Dust::Priority_Tick(_float fTimeDelta)
{
	int a = 10;
...
void CEffect_Dust::Tick(_float fTimeDelta)
{
...
void CEffect_Dust::Late_Tick(_float fTimeDelta)
{
	Compute_CamDistance();
...
HRESULT CEffect_Dust::Ready_Components()
{
	/* For.Com_Shader */
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_Effect_Dust"),
...
HRESULT CEffect_Dust::Bind_ShaderResources()
{
...
CEffect_Dust * CEffect_Dust::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CEffect_Dust*		pInstance = new CEffect_Dust(pDevice, pContext);

	/* 원형객체를 초기화한다.  */
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CEffect_Dust");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_Dust::Clone(void* pArg)
{
	CEffect_Dust*		pInstance = new CEffect_Dust(*this);

	/* 원형객체를 초기화한다.  */
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CEffect_Dust");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CEffect_Dust::Free()
{
	__super::Free();
