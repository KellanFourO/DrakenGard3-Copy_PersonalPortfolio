...
CBossPart_EN131_SwingTail::CBossPart_EN131_SwingTail(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPartObject(pDevice,pContext)
{
}

CBossPart_EN131_SwingTail::CBossPart_EN131_SwingTail(const CBossPart_EN131_SwingTail& rhs)
	: CPartObject(rhs)
{
}

HRESULT CBossPart_EN131_SwingTail::Initialize_Prototype(LEVEL eLevel)
{
	m_eCurrentLevelIndex = eLevel;
	m_strName = "CBossPart_EN131_SwingTail";
	return S_OK;
}

HRESULT CBossPart_EN131_SwingTail::Initialize(void* pArg)
{
	...
	BoundingDesc.vExtents = _float3(1.5f, 0.8f, 5.f);
...
void CBossPart_EN131_SwingTail::Priority_Tick(_float fTimeDelta)
{
	
}

void CBossPart_EN131_SwingTail::Tick(_float fTimeDelta)
{
	
}

void CBossPart_EN131_SwingTail::Late_Tick(_float fTimeDelta)
{
...
HRESULT CBossPart_EN131_SwingTail::Render()
{
	//#몬스터모델렌더
...
void CBossPart_EN131_SwingTail::Write_Json(json& Out_Json)
{
...
void CBossPart_EN131_SwingTail::Load_FromJson(const json& In_Json)
{
...
void CBossPart_EN131_SwingTail::Init_Desc()
{
	//m_pStatus.lock()->Init_Status(&m_tLinkStateDesc);
}


CBossPart_EN131_SwingTail* CBossPart_EN131_SwingTail::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LEVEL eLevel)
{
	CBossPart_EN131_SwingTail* pInstance = new CBossPart_EN131_SwingTail(pDevice, pContext);

	/* 원형객체를 초기화한다.  */
	if (FAILED(pInstance->Initialize_Prototype(eLevel)))
	{
		MSG_BOX("Failed to Created : CBossPart_EN131_SwingTail");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CBossPart_EN131_SwingTail::Clone(void* pArg)
{
	CBossPart_EN131_SwingTail* pInstance = new CBossPart_EN131_SwingTail(*this);

	/* 원형객체를 초기화한다.  */
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CBossPart_EN131_SwingTail");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CBossPart_EN131_SwingTail::Free()
{
	__super::Free();
