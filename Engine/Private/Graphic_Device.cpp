...
HRESULT CGraphic_Device::Initialize(const GRAPHIC_DESC& GraphicDesc, ID3D11Device** ppDeviceOut, ID3D11DeviceContext** ppContextOut)
{
	...
	//TODO 그래픽 장치 초기화, D11에서는 Device 컴객체와 Context 컴객체를 생성 후 셋팅해준다. 그 전에는 셋팅 후 생성 했었다.
	...
	//TODO 스왑체인 객체 생성(백버퍼텍스처 생성), 백버퍼 뷰, 뎁스스텐실 뷰 생성
	...
	//! 스왑체인이 들고 있는 텍스처 2D를 가져와서 이를 바탕으로 백버퍼 렌더타겟 뷰 생성. 렌더타겟 : 그려질 대상 == 백버퍼
	...
	//TODO 장치에 바인드해놓을 렌더타겟들과, 뎁스스텐실뷰를 셋팅. 장치는 최대 8개의 렌더타겟까지 동시에 들고 있을 수 있다.
	
	ID3D11RenderTargetView*	pRTVs[1] = //! RTV == RenderTargetView의 약자. 현재는 렌더타겟이 백 버퍼뿐이라 배열의 사이즈를 1로 선언
	{
		m_pBackBufferRTV,
	};
...
HRESULT CGraphic_Device::Ready_SwapChain(HWND hWnd, GRAPHIC_DESC::WINMODE eWinMode, _uint iWinCX, _uint iWinCY)
{
	...
	//TODO 스왑체인을 생성한다. = 텍스쳐를 생성하는 행위 + 스왑하는 형태
...
void CGraphic_Device::Free()
{
	...
	//TODO 메모리 릭이 발생한 컴 객체가 어떤 컴객체인지 알기위한 MSDN에서 제공한 코드
